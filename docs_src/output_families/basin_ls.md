...
> **What each row means:** one object's `basin_ls` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `basin_ls_day.txt` | `basin_ls_day.csv` | 2070 | 2074 | `output_landscape_init.f90:1133` |
| Monthly | `basin_ls_mon.txt` | `basin_ls_mon.csv` | 2071 | 2075 | `output_landscape_init.f90:1148` |
| Yearly | `basin_ls_yr.txt` | `basin_ls_yr.csv` | 2072 | 2076 | `output_landscape_init.f90:1163` |
| Average annual | `basin_ls_aa.txt` | `basin_ls_aa.csv` | 2073 | 2077 | `output_landscape_init.f90:1178` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (pco%` | `output_landscape_init.f90:1133` |
| Monthly | `if (time%end_mo == 1) then  →  if (pco%ls_bsn%m == "y") then` | `output_landscape_init.f90:1148` |
| Yearly | `if (time%end_yr == 1) then  →  if (pco%ls_bsn%y == "y") then` | `output_landscape_init.f90:1163` |
| Average annual | `if (time%end_sim == 1) then  →  if (pco%ls_bsn%a == "y") then` | `output_landscape_init.f90:1178` |
...
| `pco%ls_bsn%a == "y"` | aa | Enables output for this frequency. |
| `pco%ls_bsn%d == "y"` | day | Enables output for this frequency. |
| `pco%ls_bsn%m == "y"` | mon | Enables output for this frequency. |
