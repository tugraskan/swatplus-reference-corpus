...
> **What each row means:** one object's `basin_pw` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `basin_pw_day.txt` | `basin_pw_day.csv` | 2080 | 2084 | `output_landscape_init.f90:1194` |
| Monthly | `basin_pw_mon.txt` | `basin_pw_mon.csv` | 2081 | 2085 | `output_landscape_init.f90:1209` |
| Yearly | `basin_pw_yr.txt` | `basin_pw_yr.csv` | 2082 | 2086 | `output_landscape_init.f90:1224` |
| Average annual | `basin_pw_aa.txt` | `basin_pw_aa.csv` | 2083 | 2087 | `output_landscape_init.f90:1239` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (pco%` | `output_landscape_init.f90:1194` |
| Monthly | `if (time%end_mo == 1) then  →  if (pco%pw_bsn%m == "y") then` | `output_landscape_init.f90:1209` |
| Yearly | `if (time%end_yr == 1) then  →  if (pco%pw_bsn%y == "y") then` | `output_landscape_init.f90:1224` |
| Average annual | `if (time%end_sim == 1) then  →  if (pco%pw_bsn%a == "y") then` | `output_landscape_init.f90:1239` |
...
| `pco%pw_bsn%a == "y"` | aa | Enables output for this frequency. |
| `pco%pw_bsn%d == "y"` | day | Enables output for this frequency. |
| `pco%pw_bsn%m == "y"` | mon | Enables output for this frequency. |
