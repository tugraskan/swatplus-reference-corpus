...
> **What each row means:** one object's `lsu_plc_stat` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `lsu_plc_stat_day.txt` | `lsu_plc_stat_day.csv` | 4766 | 4770 | `output_landscape_init.f90:1409` |
| Monthly | `lsu_plc_stat_mon.txt` | `lsu_plc_stat_mon.csv` | 4767 | 4771 | `output_landscape_init.f90:1421` |
| Yearly | `lsu_plc_stat_yr.txt` | `lsu_plc_stat_yr.csv` | 4768 | 4772 | `output_landscape_init.f90:1433` |
| Average annual | `lsu_plc_stat_aa.txt` | `lsu_plc_stat_aa.csv` | 4769 | 4773 | `output_landscape_init.f90:1445` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `do ilsu = 1, db_mx%lsu_out  →  if (pco%cb_plt_lsu%d == "y") then` | `output_landscape_init.f90:1409` |
| Monthly | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_mo == 1) then  →  if (pco%cb_plt_lsu` | `output_landscape_init.f90:1421` |
| Yearly | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_yr == 1) then  →  if (pco%cb_plt_lsu` | `output_landscape_init.f90:1433` |
| Average annual | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_sim == 1) then  →  if (pco%cb_gl_lsu` | `output_landscape_init.f90:1445` |
...
| `pco%cb_plt_lsu%a == "y"` | aa | Enables output for this frequency. |
| `pco%cb_plt_lsu%d == "y"` | day | Enables output for this frequency. |
| `pco%cb_plt_lsu%m == "y"` | mon | Enables output for this frequency. |
| `pco%cb_plt_lsu%y == "y"` | yr | Enables output for this frequency. |
