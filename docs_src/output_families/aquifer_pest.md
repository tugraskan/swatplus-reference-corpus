...
title: aquifer_pest_*
status: filled
source_hash: d05609836c6d68cd
version_label: SWAT+ 62.0.0
---

**Kind:** output family  
**Opened by:** [`header_pest`](../procedures/header_pest.md)  
**Written by:** [`aqu_pesticide_output`](../procedures/aqu_pesticide_output.md)  
**Primary data type:** `aqu_pesticide_module::aqu_pesticide_processes`  
**Files covered:** `aquifer_pest_day`, `aquifer_pest_mon`, `aquifer_pest_yr`, `aquifer_pest_aa` text/CSV pairs

## Bottom Line

`aquifer_pest_*` is the `aquifer_pest` pesticide time-series output family. Every file shares one record layout, so daily, monthly, yearly, and average-annual reporting live on one page. Unlike the single-value families, `aqu_pesticide_output` loops over every simulated pesticide and writes **one row per (object x pesticide)** for each period: the row carries the time and object-identity fields, the pesticide name, and a `aqu_pesticide_processes` balance record.
...
> **What each row means:** the `aquifer_pest` balance of one pesticide for one object over one reporting period (daily, monthly, yearly, average annual). If a run simulates several pesticides, each object appears once per pesticide per period. The columns are the same in every file of the family.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `aquifer_pest_day.txt` | `aquifer_pest_day.csv` | 3008 | 3012 | `header_pest.f90:274` |
| Monthly | `aquifer_pest_mon.txt` | `aquifer_pest_mon.csv` | 3009 | 3013 | `header_pest.f90:289` |
| Yearly | `aquifer_pest_yr.txt` | `aquifer_pest_yr.csv` | 3010 | 3014 | `header_pest.f90:304` |
| Average annual | `aquifer_pest_aa.txt` | `aquifer_pest_aa.csv` | 3011 | 3015 | `header_pest.f90:319` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `do ipest = 1, cs_db%num_pests  →  if (pco%day_print == "y" .and. pco%int_day_cur` | `header_pest.f90:274` |
| Monthly | `do ipest = 1, cs_db%num_pests  →  if (time%end_mo == 1) then  →  if (pco%pest%m ` | `header_pest.f90:289` |
| Yearly | `do ipest = 1, cs_db%num_pests  →  if (time%end_yr == 1) then  →  if (time%end_yr` | `header_pest.f90:304` |
| Average annual | `do ipest = 1, cs_db%num_pests  →  if (time%end_sim == 1 .and. pco%pest%a == "y")` | `header_pest.f90:319` |
...
| Data row | `aqu_pesticide_output` | One `aqu_pesticide_processes` record for one pesticide at the active frequency. |
...
| `tot_in` | kg | `aqupst_d%pest(i)%tot_in` | total pesticide into aquifer |
| `sol_flo` | kg | `aqupst_d%pest(i)%sol_flo` | soluble pesticide out of aquifer |
| `sor_flo` | kg | `aqupst_d%pest(i)%sor_flo` | sorbed pesticide out of aquifer |
| `sol_perc` | kg | `aqupst_d%pest(i)%sol_perc` | sorbed pesticide out of aquifer |
| `react` | kg | `aqupst_d%pest(i)%react` | pesticide lost through reactions |
| `metab` | kg | `aqupst_d%pest(i)%metab` | amount of pesticide metabolized from parent |
| `stor_ave` | kg | `aqupst_d%pest(i)%stor_ave` | average end of day pesticide in aquifer during the time period |
| `stor_init` | kg | `aqupst_d%pest(i)%stor_init` | pesticide in aquifer at the start of the day |
| `stor_final` | kg | `aqupst_d%pest(i)%stor_final` | pesticide in aquifer at the end of the day |
...
|---|---|---|
| `aqu_pesticide_output.f90:34` | `3008` | time, identity, pesticide name, one `aqu_pesticide_processes` record |
| `aqu_pesticide_output.f90:37` | `3012` | time, identity, pesticide name, one `aqu_pesticide_processes` record |
| `aqu_pesticide_output.f90:52` | `3009` | time, identity, pesticide name, one `aqu_pesticide_processes` record |
| `aqu_pesticide_output.f90:55` | `3013` | time, identity, pesticide name, one `aqu_pesticide_processes` record |
| `aqu_pesticide_output.f90:74` | `3010` | time, identity, pesticide name, one `aqu_pesticide_processes` record |
| `aqu_pesticide_output.f90:77` | `3014` | time, identity, pesticide name, one `aqu_pesticide_processes` record |
| `aqu_pesticide_output.f90:92` | `3011` | time, identity, pesticide name, one `aqu_pesticide_processes` record |
| `aqu_pesticide_output.f90:95` | `3015` | time, identity, pesticide name, one `aqu_pesticide_processes` record |
...
- Writer: [`aqu_pesticide_output`](../procedures/aqu_pesticide_output.md)
