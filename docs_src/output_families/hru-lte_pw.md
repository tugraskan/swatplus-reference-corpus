...
> **What each row means:** one object's `hru-lte_pw` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `hru-lte_pw_day.txt` | `hru-lte_pw_day.csv` | 2460 | 2464 | `output_landscape_init.f90:699` |
| Monthly | `hru-lte_pw_mon.txt` | `hru-lte_pw_mon.csv` | 2461 | 2465 | `output_landscape_init.f90:714` |
| Yearly | `hru-lte_pw_yr.txt` | `hru-lte_pw_yr.csv` | 2462 | 2466 | `output_landscape_init.f90:729` |
| Average annual | `hru-lte_pw_aa.txt` | `hru-lte_pw_aa.csv` | 2463 | 2467 | `output_landscape_init.f90:744` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (pco%` | `output_landscape_init.f90:699` |
| Monthly | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (time` | `output_landscape_init.f90:714` |
| Yearly | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (time` | `output_landscape_init.f90:729` |
| Average annual | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (time` | `output_landscape_init.f90:744` |
...
| `pco%pw_sd%m == "y"` | mon | Enables output for this frequency. |
| `time%end_mo == 1` | mon | Builds and writes rows at month end. |
| `time%end_sim == 1 .and. pco%pw_sd%a == "y"` | aa | Open/print guard. |
| `time%end_yr == 1` | yr | Builds and writes rows at year end. |
| `time%end_yr == 1 .and. pco%pw_sd%y == "y"` | yr | Builds and writes rows at year end. |
