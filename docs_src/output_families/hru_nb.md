...
> **What each row means:** one object's `hru_nb` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `hru_nb_day.txt` | `hru_nb_day.csv` | 2020 | 2024 | `output_landscape_init.f90:103` |
| Monthly | `hru_nb_mon.txt` | `hru_nb_mon.csv` | 2021 | 2025 | `output_landscape_init.f90:180` |
| Yearly | `hru_nb_yr.txt` | `hru_nb_yr.csv` | 2022 | 2026 | `output_landscape_init.f90:195` |
| Average annual | `hru_nb_aa.txt` | `hru_nb_aa.csv` | 2023 | 2027 | `output_landscape_init.f90:210` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (pco%` | `output_landscape_init.f90:103` |
| Monthly | `if (time%end_mo == 1) then  →  if (pco%nb_hru%m == "y") then` | `output_landscape_init.f90:180` |
| Yearly | `if (time%end_yr == 1) then  →  if (pco%nb_hru%y == "y") then` | `output_landscape_init.f90:195` |
| Average annual | `if (time%end_sim == 1 .and. pco%nb_hru%a == "y") then` | `output_landscape_init.f90:210` |
...
| `pco%nb_hru%m == "y"` | mon | Enables output for this frequency. |
| `pco%nb_hru%y == "y"` | yr | Enables output for this frequency. |
| `time%end_mo == 1` | mon | Builds and writes rows at month end. |
| `time%end_sim == 1 .and. pco%nb_hru%a == "y"` | aa | Open/print guard. |
