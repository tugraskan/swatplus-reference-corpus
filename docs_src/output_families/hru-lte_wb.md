...
> **What each row means:** one object's `hru-lte_wb` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `hru-lte_wb_day.txt` | `hru-lte_wb_day.csv` | 2300 | 2304 | `output_landscape_init.f90:559` |
| Monthly | `hru-lte_wb_mon.txt` | `hru-lte_wb_mon.csv` | 2301 | 2305 | `output_landscape_init.f90:575` |
| Yearly | `hru-lte_wb_yr.txt` | `hru-lte_wb_yr.csv` | 2302 | 2306 | `output_landscape_init.f90:592` |
| Average annual | `hru-lte_wb_aa.txt` | `hru-lte_wb_aa.csv` | 2303 | 2307 | `output_landscape_init.f90:609` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (pco%` | `output_landscape_init.f90:559` |
| Monthly | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (time` | `output_landscape_init.f90:575` |
| Yearly | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (time` | `output_landscape_init.f90:592` |
| Average annual | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (time` | `output_landscape_init.f90:609` |
...
| `pco%wb_sd%m == "y"` | mon | Enables output for this frequency. |
| `time%end_mo == 1` | mon | Builds and writes rows at month end. |
| `time%end_sim == 1 .and. pco%wb_sd%a == "y"` | aa | Open/print guard. |
| `time%end_yr == 1` | yr | Builds and writes rows at year end. |
| `time%end_yr == 1 .and. pco%wb_sd%y == "y"` | yr | Builds and writes rows at year end. |
