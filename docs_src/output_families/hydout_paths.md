...
> **What each row means:** one outgoing hydrograph link leaving one object for one reporting period. The first columns say which object it is and where the outflow goes (destination object type/number, hydrograph type, and the fraction of this object's hydrograph sent on this link); the remaining columns give the mass of each simulated pathogen carried out on that link. If a run simulates N pathogens there are N constituent columns. Pick the file whose frequency matches your timestep — the columns are the same across all four. Whole files only exist when at least one pathogen is simulated (`cs_db%num_paths > 0`).

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | State Written | Write Lines |
|---|---|---|---:|---:|---|---|
| Daily | `hydout_paths_day.txt` | `hydout_paths_day.csv` | 2744 | 2760 | `hcs1%path` | `hcsout_output.f90:33, 37` |
| Monthly | `hydout_paths_mon.txt` | `hydout_paths_mon.csv` | 2745 | 2761 | `obcs(iob)%hcsout_m(iiout)%path` | `hcsout_output.f90:84, 88` |
| Yearly | `hydout_paths_yr.txt` | `hydout_paths_yr.csv` | 2746 | 2762 | `obcs(iob)%hcsout_y(iiout)%path` | `hcsout_output.f90:135, 139` |
| Average annual | `hydout_paths_aa.txt` | `hydout_paths_aa.csv` | 2747 | 2763 | `obcs(iob)%hcsout_a(iiout)%path` | `hcsout_output.f90:186, 190` |

## File Contracts

| Frequency | Open Condition | Open Lines | Header Row | Catalog Entry |
|---|---|---|---|---|
| Daily | `pco%hyd%d == "y"` and `cs_db%num_paths > 0` | `header_cs.f90:252` (txt), `:257` (csv) | `csout_hyd_hdr` + one `cs_path_solsor` label group per pathogen (`header_cs.f90:255`) | `HYDOUT_PATHS hydout_paths_day.txt/csv` |
| Monthly | `pco%hyd%m == "y"` and `cs_db%num_paths > 0` | `header_cs.f90:307` (txt), `:312` (csv) | same | `HYDOUT_PATHS hydout_paths_mon.txt/csv` |
| Yearly | `pco%hyd%y == "y"` and `cs_db%num_paths > 0` | `header_cs.f90:363` (txt), `:368` (csv) | same (see note) | `HYDOUT_PATHS hydout_paths_yr.txt/csv` |
| Average annual | `pco%hyd%a == "y"` and `cs_db%num_paths > 0` | `header_cs.f90:418` (txt), `:423` (csv) | same (see note) | `HYDOUT_PATHS hydout_paths_aa.txt/csv` |

## Writer And Print Controls

| Control | Source Line | Applies To | Meaning |
|---|---:|---|---|
| `cs_db%num_paths > 0` | `hcsout_output.f90:32`, `header_cs.f90:251` | All | Nothing is opened or written unless at least one pathogen is simulated. |
...
| `pco%csvout == "y"` | `hcsout_output.f90:36`, `header_cs.f90:256` | CSV companions | Enables the CSV file beside each text file. |
...
|---|---|---|---|
| Title row | Basin name and program string | `header_cs.f90:254` | Identifies the model run. |
| Header row | `csout_hyd_hdr` + per-pathogen labels | `header_cs.f90:255` | Column names; see the note about label/value alignment in Review Notes. |
| Data row | time, object identity, outflow descriptors, per-pathogen mass | `hcsout_output.f90:33` | One outgoing hydrograph link for the active frequency. |

```text
title:   bsn%name, prog
header:  csout_hyd_hdr, (cs_path_solsor(ipath), ipath = 1, num_paths)
data:    jday mon day yr iob gis_id type num obtypout obtyp_noout htyp_out frac_out  path(1) ... path(num_paths)
```
...
| `path(ipath)` | cfu | `hcs1%path(ipath)` / `hcsout_*(iiout)%path(ipath)` | Mass of pathogen `ipath` carried out on this outgoing hydrograph. One column per simulated pathogen. |
...
| Text/CSV units | 2744 / 2760 | 2745 / 2761 | 2746 / 2762 | 2747 / 2763 |
...
- These are running sums of pathogen mass; the writer does not divide the constituent columns by the number of days or years, so monthly/yearly/average-annual rows are period totals of the daily outflow.
...
3. If daily printing is enabled and within the print interval and `cs_db%num_paths > 0`, write the daily row from `hcs1` to unit 2744 (and 2760 for CSV).
...
|---:|---|---|---|---|
| `hcsout_output.f90:33` | `write` | `2744` | `hydout_paths_day.txt` | time, object identity, outflow descriptors, `hcs1%path(:)` |
| `hcsout_output.f90:37` | `write` | `2760` | `hydout_paths_day.csv` | time, object identity, outflow descriptors, `hcs1%path(:)` |
| `hcsout_output.f90:84` | `write` | `2745` | `hydout_paths_mon.txt` | time, object identity, outflow descriptors, `hcsout_m(iiout)%path(:)` |
| `hcsout_output.f90:135` | `write` | `2746` | `hydout_paths_yr.txt` | time, object identity, outflow descriptors, `hcsout_y(iiout)%path(:)` |
| `hcsout_output.f90:186` | `write` | `2747` | `hydout_paths_aa.txt` | time, object identity, outflow descriptors, `hcsout_a(iiout)%path(:)` |

## Review Notes

- **Header/data column alignment.** The fixed part of the header (`csout_hyd_hdr`, `constituent_mass_module.f90:568-581`) has twelve labels that match the twelve fixed data columns. Each pathogen then contributes a `cs_path_solsor` group (`sol_out`/`sor_out`, two labels) to the header, while the data row writes a single value per pathogen. Readers aligning constituent columns by position should key off the writer (`hcsout_output.f90:33`), not the per-constituent header labels.
- **Header constituent-name quirk.** The yearly HYDOUT_PATHS header appends `cs_pest_solsor` rather than `cs_path_solsor` (`header_cs.f90:366`), and the average-annual header loops `cs_path_solsor(ipest)` with the `ipath` counter (`header_cs.f90:421`). These are source-level naming/index slips in the header line only; they affect the constituent-name labels, not the data values written by `hcsout_output`.
