...
> **What each row means:** one object's `hru_pw` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `hru_pw_day.txt` | `hru_pw_day.csv` | 2040 | 2044 | `output_landscape_init.f90:495` |
| Monthly | `hru_pw_mon.txt` | `hru_pw_mon.csv` | 2041 | 2045 | `output_landscape_init.f90:510` |
| Yearly | `hru_pw_yr.txt` | `hru_pw_yr.csv` | 2042 | 2046 | `output_landscape_init.f90:525` |
| Average annual | `hru_pw_aa.txt` | `hru_pw_aa.csv` | 2043 | 2047 | `output_landscape_init.f90:540` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then  →  if (pco%` | `output_landscape_init.f90:495` |
| Monthly | `if (time%end_mo == 1) then  →  if (pco%pw_hru%m == "y") then` | `output_landscape_init.f90:510` |
| Yearly | `if (time%end_yr == 1) then  →  if (pco%pw_hru%y == "y") then` | `output_landscape_init.f90:525` |
| Average annual | `if (time%end_sim == 1 .and. pco%pw_hru%a == "y") then` | `output_landscape_init.f90:540` |
...
| `pco%pw_hru%m == "y"` | mon | Enables output for this frequency. |
| `pco%pw_hru%y == "y"` | yr | Enables output for this frequency. |
| `time%end_mo == 1` | mon | Builds and writes rows at month end. |
| `time%end_sim == 1 .and. pco%pw_hru%a == "y"` | aa | Open/print guard. |
