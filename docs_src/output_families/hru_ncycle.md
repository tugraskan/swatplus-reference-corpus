...
> **What each row means:** one object's `hru_ncycle` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `hru_ncycle_day.txt` | `hru_ncycle_day.csv` | 3333 | 3334 | `output_landscape_init.f90:119` |
| Monthly | `hru_ncycle_mon.txt` | `hru_ncycle_mon.csv` | 3335 | 3336 | `output_landscape_init.f90:134` |
| Yearly | `hru_ncycle_yr.txt` | `hru_ncycle_yr.csv` | 3337 | 3338 | `output_landscape_init.f90:149` |
| Average annual | `hru_ncycle_aa.txt` | `hru_ncycle_aa.csv` | 3339 | 3340 | `output_landscape_init.f90:164` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `if (sp_ob%hru  →  0) then  →  if (pco%wb_hru%d == "y") then  →  if (pco%wb_hru%y` | `output_landscape_init.f90:119` |
| Monthly | `if (sp_ob%hru  →  0) then  →  if (pco%wb_hru%d == "y") then  →  if (pco%wb_hru%y` | `output_landscape_init.f90:134` |
| Yearly | `if (sp_ob%hru  →  0) then  →  if (pco%wb_hru%d == "y") then  →  if (pco%wb_hru%y` | `output_landscape_init.f90:149` |
| Average annual | `if (sp_ob%hru  →  0) then  →  if (pco%wb_hru%d == "y") then  →  if (pco%wb_hru%y` | `output_landscape_init.f90:164` |
...
| `pco%nb_hru%a == "y"` | aa | Enables output for this frequency. |
| `pco%nb_hru%d == "y"` | All files | Enables output for this frequency. |
| `pco%nb_hru%m == "y"` | aa, mon, yr | Enables output for this frequency. |
| `pco%nb_hru%y == "y"` | aa, yr | Enables output for this frequency. |
...
The columns are identical for every frequency (daily, monthly, yearly, average annual). What differs is only the file name, unit number, print flag, and the source state object (`wb_hdr` for daily and its monthly/yearly/average-annual counterparts). See the Output Family and Writer And Print Controls tables above for the per-frequency file, unit, and trigger.
