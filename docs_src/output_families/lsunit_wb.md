...
> **What each row means:** one object's `lsunit_wb` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `lsunit_wb_day.txt` | `lsunit_wb_day.csv` | 2140 | 2144 | `output_landscape_init.f90:762` |
| Monthly | `lsunit_wb_mon.txt` | `lsunit_wb_mon.csv` | 2141 | 2145 | `output_landscape_init.f90:778` |
| Yearly | `lsunit_wb_yr.txt` | `lsunit_wb_yr.csv` | 2142 | 2146 | `output_landscape_init.f90:794` |
| Average annual | `lsunit_wb_aa.txt` | `lsunit_wb_aa.csv` | 2143 | 2147 | `output_landscape_init.f90:810` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `do ilsu = 1, db_mx%lsu_out  →  if (pco%day_print == "y" .and. pco%int_day_cur ==` | `output_landscape_init.f90:762` |
| Monthly | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_mo == 1) then  →  if (pco%wb_lsu%m =` | `output_landscape_init.f90:778` |
| Yearly | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_yr == 1) then  →  if (pco%wb_lsu%y =` | `output_landscape_init.f90:794` |
| Average annual | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_sim == 1 .and. pco%wb_lsu%a == "y") ` | `output_landscape_init.f90:810` |
...
| `pco%wb_lsu%m == "y"` | mon | Enables output for this frequency. |
| `pco%wb_lsu%y == "y"` | yr | Enables output for this frequency. |
| `time%end_mo == 1` | mon | Builds and writes rows at month end. |
| `time%end_sim == 1 .and. pco%wb_lsu%a == "y"` | aa | Open/print guard. |
