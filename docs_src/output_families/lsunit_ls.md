...
> **What each row means:** one object's `lsunit_ls` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `lsunit_ls_day.txt` | `lsunit_ls_day.csv` | 2160 | 2164 | `output_landscape_init.f90:887` |
| Monthly | `lsunit_ls_mon.txt` | `lsunit_ls_mon.csv` | 2161 | 2165 | `output_landscape_init.f90:902` |
| Yearly | `lsunit_ls_yr.txt` | `lsunit_ls_yr.csv` | 2162 | 2166 | `output_landscape_init.f90:917` |
| Average annual | `lsunit_ls_aa.txt` | `lsunit_ls_aa.csv` | 2163 | 2167 | `output_landscape_init.f90:932` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `do ilsu = 1, db_mx%lsu_out  →  if (pco%day_print == "y" .and. pco%int_day_cur ==` | `output_landscape_init.f90:887` |
| Monthly | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_mo == 1) then  →  if (pco%ls_lsu%m =` | `output_landscape_init.f90:902` |
| Yearly | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_yr == 1) then  →  if (pco%ls_lsu%y =` | `output_landscape_init.f90:917` |
| Average annual | `do ilsu = 1, db_mx%lsu_out  →  if (time%end_sim == 1 .and. pco%ls_lsu%a == "y") ` | `output_landscape_init.f90:932` |
...
| `pco%ls_lsu%m == "y"` | mon | Enables output for this frequency. |
| `pco%ls_lsu%y == "y"` | yr | Enables output for this frequency. |
| `time%end_mo == 1` | mon | Builds and writes rows at month end. |
| `time%end_sim == 1 .and. pco%ls_lsu%a == "y"` | aa | Open/print guard. |
