...
> **What each row means:** one object's `hru_carb_gl` values for a single reporting period (daily, monthly, yearly, average annual). Every file in the family has the same columns — pick the one whose frequency matches the timestep you want.

## Output Family

| Frequency | Text File | CSV File | Text Unit | CSV Unit | Open Lines |
|---|---|---|---:|---:|---|
| Daily | `hru_carb_gl_day.txt` | `hru_carb_gl_day.csv` | 4520 | 4524 | `output_landscape_init.f90:226` |
| Monthly | `hru_carb_gl_mon.txt` | `hru_carb_gl_mon.csv` | 4521 | 4525 | `output_landscape_init.f90:241` |
| Yearly | `hru_carb_gl_yr.txt` | `hru_carb_gl_yr.csv` | 4522 | 4526 | `output_landscape_init.f90:256` |
| Average annual | `hru_carb_gl_aa.txt` | `hru_carb_gl_aa.csv` | 4523 | 4527 | `output_landscape_init.f90:271` |

## File Contracts

| Frequency | Open Condition | Open Lines |
|---|---|---|
| Daily | `if (pco%cb_gl_hru%d == "y") then` | `output_landscape_init.f90:226` |
| Monthly | `if (time%end_mo == 1) then  →  if (pco%cb_gl_hru%m == "y") then` | `output_landscape_init.f90:241` |
| Yearly | `if (time%end_yr == 1) then  →  if (pco%cb_gl_hru%y == "y") then` | `output_landscape_init.f90:256` |
| Average annual | `if (time%end_sim == 1) then  →  if (pco%cb_gl_hru%a == "y" .or. pco%cb_trf_hru%a` | `output_landscape_init.f90:271` |
...
| `pco%cb_gl_hru%a == "y"` | aa | Enables output for this frequency. |
...
| `pco%cb_gl_hru%m == "y"` | aa, mon, yr | Enables output for this frequency. |
| `pco%cb_gl_hru%y == "y"` | aa, yr | Enables output for this frequency. |
