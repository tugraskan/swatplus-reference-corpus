---
kind: procedure
symbol: calsoft_hyd_bfr_pet
title: calsoft_hyd_bfr_pet
status: filled
source_hash: 9a6c005437b14c13
...
locals:
  ...
  pred: Local counter/scalar used in the calibration loop (`pred`).
uses:
...
Soft-calibrates PET (before flow-regime partitioning) by adjusting pet_co (PET) per region/land-use to match the measured ratio.

## Bottom Line

`calsoft_hyd_bfr_pet` is the before-flow-regime soft-calibration for PET. For each region/land use it compares the simulated vs measured PET statistic and adjusts pet_co (PET), clamping to limits.
...
Part of the soft/hard calibration sequence (driven by `calsoft_control`/`calsoft_hyd_bfr`). It adjusts parameters to calibrate PET (before-flow-regime), re-initializes objects, and re-runs the model via `time_control`. It runs only when calibration is enabled, before the production simulation.
...
| --- | --- | --- |
| [sym:hru_module] | `hru, hru_init` | `hru(iihru)%lum_group_c, hru(iihru)%hyd%pet_co, hru_init(iihru)%hyd%pet_co` |
...
| [sym:calibration_data_module] | `region, lscal, ls_prms, lscal_z` | `region(ireg)%nlum, lscal(ireg)%lum(ilum)%meas%wyr, lscal(ireg)%lum(ilum)%precip_aa, lscal(ireg)%lum(ilum)%aa%wyr, lscal(ireg)%lum(ilum)%ha, lscal(ireg)%lum(ilum)%prm_prev, lscal(ireg)%lum(ilum)%prm, lscal(ireg)%lum(ilum)%prev, lscal(ireg)%lum(ilum)%aa, lscal(ireg)%lum(ilum)%prm_prev%petco, lscal(ireg)%lum(ilum)%prm%petco, ls_prms(4)%pos, lscal(ireg)%lum(ilum)%prm_lim%petco, ls_prms(4)%neg, region(ireg)%num_tot, region(ireg)%num(ihru_s), lscal(ireg)%lum(ilum)%meas%name, ls_prms(4)%up, ls_prms(4)%lo, lscal(ireg)%lum(ilum)%nbyr` |
...
`calsoft_hyd_bfr_pet.f90` was introduced in `e18817a` (2024-10-08, "Refactor and enhance various modules and subroutines") and has been changed in 3 non-merge commit(s) since, most recently `bd18ad4` (2025-11-24, "Pr 107 (#108)"). Lineage is reconstructed from commit metadata (SHA, date, subject); per-line diffs were unavailable in this build environment, so only the commits touching `calsoft_hyd_bfr_pet.f90` are listed.
...
- warning: missing_doc: Procedure 'calsoft_hyd_bfr_pet' has no extracted documentation comment.
- Soft-calibration routine for PET (before-flow-regime): compares simulated vs measured statistics, adjusts parameters, and re-runs via time_control. 11 use-imported modules had no specific symbol resolved.
