...
`aqupest_add` returns a new `aqu_pesticide_processes` record whose fields are the field-by-field sum of its two inputs. All 7 of the record's 9 value fields are combined with `+`. 2 fields are carried through unchanged rather than combined: `stor_init`, `stor_final`.
...
- warning: missing_doc: Procedure 'aqu_pesticide_module::aqupest_add' has no extracted documentation comment.
