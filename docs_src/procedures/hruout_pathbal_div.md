---
kind: procedure
symbol: hruout_pathbal_div
title: hruout_pathbal_div
status: filled
source_hash: f797054712c9dc39
...
`hruout_pathbal_div` returns a new `pathogen_balance` record whose fields are its input's fields each divided by the scalar `const`. 9 of the 9 fields are divided by `const`: `plant`, `soil`, `sed`, `surq`, `perc1`, `apply_sol`, `apply_plt`, `regro`, `die_off`.
...
`hruout_pathbal_div` is a pure, side-effect-free helper in `output_ls_pathogen_module`; it only computes a new `pathogen_balance` value from its arguments and does no I/O. It runs wherever `pathogen_balance` records are averaged, chiefly along the output-accumulation path (daily → monthly → yearly → average annual).
...
- warning: missing_doc: Procedure 'output_ls_pathogen_module::hruout_pathbal_div' has no extracted documentation comment.
