...
`aqupest_ave` returns a new `aqu_pesticide_processes` record whose fields are its input's fields each divided by the scalar `const`. 1 of the 9 fields are divided by `const`: `stor_ave`. The other 8 fields are copied through unchanged (not scaled); these are the flux totals that are not averaged.
...
`aqupest_ave` is a pure, side-effect-free helper in `aqu_pesticide_module`; it only computes a new `aqu_pesticide_processes` value from its arguments and does no I/O. It runs wherever `aqu_pesticide_processes` records are averaged, chiefly along the output-accumulation path (daily → monthly → yearly → average annual).
...
- warning: missing_doc: Procedure 'aqu_pesticide_module::aqupest_ave' has no extracted documentation comment.
