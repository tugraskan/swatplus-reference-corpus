---
kind: procedure
symbol: calsoft_hyd_bfr_et
title: calsoft_hyd_bfr_et
status: filled
source_hash: f3df7b6857983981
...
Soft-calibrates ET (before flow-regime partitioning) by adjusting esco (ET) per region/land-use to match the measured ratio.

## Bottom Line

`calsoft_hyd_bfr_et` is the before-flow-regime soft-calibration for ET. For each region/land use it compares the simulated vs measured ET statistic and adjusts esco (ET), clamping to limits.
...
Part of the soft/hard calibration sequence (driven by `calsoft_control`/`calsoft_hyd_bfr`). It adjusts parameters to calibrate ET (before-flow-regime), re-initializes objects, and re-runs the model via `time_control`. It runs only when calibration is enabled, before the production simulation.
...
| --- | --- | --- |
| [sym:hru_module] | `hru, hru_init` | `hru(iihru)%lum_group_c, hru(iihru)%hyd%esco, hru_init(iihru)%hyd%esco` |
...
| [sym:calibration_data_module] | `region, lscal, ls_prms, lscal_z` | `region(ireg)%nlum, lscal(ireg)%lum(ilum)%meas%wyr, lscal(ireg)%lum(ilum)%meas%pcr, lscal(ireg)%lum(ilum)%precip_aa, lscal(ireg)%lum(ilum)%aa%wyr, lscal(ireg)%lum(ilum)%ha, lscal(ireg)%lum(ilum)%prm_lim%etco, lscal(ireg)%lum(ilum)%prm_prev, lscal(ireg)%lum(ilum)%prm, lscal(ireg)%lum(ilum)%prev, lscal(ireg)%lum(ilum)%aa, lscal(ireg)%lum(ilum)%prm_prev%etco, lscal(ireg)%lum(ilum)%prm%etco, lscal(ireg)%lum(ilum)%prev%wyr, ls_prms(2)%pos, ls_prms(2)%neg, region(ireg)%num_tot, region(ireg)%num(ihru_s), lscal(ireg)%lum(ilum)%meas%name, ls_prms(2)%up, ls_prms(2)%lo, lscal(ireg)%lum(ilum)%nbyr` |
...
| `lscal(ireg)%lum(ilum)%prev%wyr` | When the simulated statistic differs from the measured target beyond the tolerance for a region/land-use. | Saves the previous simulated water-yield ratio for the calibration unit. |
...
- warning: missing_doc: Procedure 'calsoft_hyd_bfr_et' has no extracted documentation comment.
- Soft-calibration routine for ET (before-flow-regime): compares simulated vs measured statistics, adjusts parameters, and re-runs via time_control. 11 use-imported modules had no specific symbol resolved.
