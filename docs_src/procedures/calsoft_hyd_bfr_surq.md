---
kind: procedure
symbol: calsoft_hyd_bfr_surq
title: calsoft_hyd_bfr_surq
status: filled
source_hash: 863281620df75bd9
...
Soft-calibrates surface runoff (before flow-regime partitioning) by adjusting cn3_swf (surface runoff) per region/land-use to match the measured ratio.

## Bottom Line

`calsoft_hyd_bfr_surq` is the before-flow-regime soft-calibration for surface runoff. For each region/land use it compares the simulated vs measured surface runoff statistic and adjusts cn3_swf (surface runoff), clamping to limits.
...
Part of the soft/hard calibration sequence (driven by `calsoft_control`/`calsoft_hyd_bfr`). It adjusts parameters to calibrate surface runoff (before-flow-regime), re-initializes objects, and re-runs the model via `time_control`. It runs only when calibration is enabled, before the production simulation.
...
| --- | --- | --- |
| [sym:hru_module] | `hru, hru_init, cn2` | `hru(iihru)%lum_group_c, hru(iihru)%tiledrain, hru(iihru)%hyd%cn3_swf, hru_init(iihru)%hyd%cn3_swf` |
...
| [sym:calibration_data_module] | `region, lscal, ls_prms, lscal_z` | `region(ireg)%nlum, lscal(ireg)%lum(ilum)%meas%srr, lscal(ireg)%lum(ilum)%precip_aa, lscal(ireg)%lum(ilum)%ha, lscal(ireg)%lum(ilum)%prm_lim%cn3_swf, lscal(ireg)%lum(ilum)%prm_prev, lscal(ireg)%lum(ilum)%prm, lscal(ireg)%lum(ilum)%prev, lscal(ireg)%lum(ilum)%aa, lscal(ireg)%lum(ilum)%aa%srr, lscal(ireg)%lum(ilum)%prm_prev%cn3_swf, lscal(ireg)%lum(ilum)%prm%cn3_swf, lscal(ireg)%lum(ilum)%prev%srr, ls_prms(10)%pos, ls_prms(10)%neg, region(ireg)%num_tot, region(ireg)%num(ihru_s), lscal(ireg)%lum(ilum)%meas%name, ls_prms(10)%up, ls_prms(10)%lo, lscal(ireg)%lum(ilum)%nbyr` |
...
- warning: missing_doc: Procedure 'calsoft_hyd_bfr_surq' has no extracted documentation comment.
- Soft-calibration routine for surface runoff (before-flow-regime): compares simulated vs measured statistics, adjusts parameters, and re-runs via time_control. 11 use-imported modules had no specific symbol resolved.
