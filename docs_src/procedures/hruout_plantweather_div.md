---
kind: procedure
symbol: hruout_plantweather_div
title: hruout_plantweather_div
status: filled
source_hash: 137d8a5e571d1a30
...
`hruout_plantweather_div` returns a new `output_plantweather` record whose fields are its input's fields each divided by the scalar `const`. 8 of the 25 fields are divided by `const`: `yield`, `strsw`, `strsa`, `strstmp`, `strsn`, `strsp`, `strss`, `percn`. The other 17 fields are copied through unchanged (not scaled); these are the flux totals that are not averaged.
...
- warning: missing_doc: Procedure 'output_landscape_module::hruout_plantweather_div' has no extracted documentation comment.
