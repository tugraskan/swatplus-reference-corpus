---
kind: procedure
symbol: hruout_pestbal_ave
title: hruout_pestbal_ave
status: filled
source_hash: 8e488ccf2086633a
...
`hruout_pestbal_ave` returns a new `pesticide_balance` record whose fields are its input's fields each divided by the scalar `const`. 3 of the 16 fields are divided by `const`: `plant`, `soil`, `in_plant`. The other 13 fields are copied through unchanged (not scaled); these are the flux totals that are not averaged.
...
`hruout_pestbal_ave` is a pure, side-effect-free helper in `output_ls_pesticide_module`; it only computes a new `pesticide_balance` value from its arguments and does no I/O. It runs wherever `pesticide_balance` records are averaged, chiefly along the output-accumulation path (daily → monthly → yearly → average annual).
...
- warning: missing_doc: Procedure 'output_ls_pesticide_module::hruout_pestbal_ave' has no extracted documentation comment.
