---
kind: procedure
symbol: hruout_saltbal_div
title: hruout_saltbal_div
status: filled
source_hash: 7cfa2a5348c52b84
...
`hruout_saltbal_div` returns a new `salt_balance` record whose fields are its input's fields each divided by the scalar `const`. 5 of the 7 fields are divided by `const`: `surq`, `latq`, `tileq`, `perc`, `irrig`. The other 2 fields are copied through unchanged (not scaled); these are the flux totals that are not averaged.
...
`hruout_saltbal_div` is a pure, side-effect-free helper in `output_ls_salt_module`; it only computes a new `salt_balance` value from its arguments and does no I/O. It runs wherever `salt_balance` records are averaged, chiefly along the output-accumulation path (daily → monthly → yearly → average annual).
...
- warning: missing_doc: Procedure 'output_ls_salt_module::hruout_saltbal_div' has no extracted documentation comment.
