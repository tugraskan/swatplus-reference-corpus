---
kind: procedure
symbol: hruout_waterbal_ave
title: hruout_waterbal_ave
status: filled
source_hash: 0205a51ed7b6e97a
...
`hruout_waterbal_ave` returns a new `output_waterbal` record whose fields are its input's fields each divided by the scalar `const`. 5 of the 42 fields are divided by `const`: `cn`, `sw`, `sw_300`, `snopack`, `wet_stor`. The other 37 fields are copied through unchanged (not scaled); these are the flux totals that are not averaged.
...
`hruout_waterbal_ave` is a pure, side-effect-free helper in `output_landscape_module`; it only computes a new `output_waterbal` value from its arguments and does no I/O. It runs wherever `output_waterbal` records are averaged, chiefly along the output-accumulation path (daily → monthly → yearly → average annual).
...
- warning: missing_doc: Procedure 'output_landscape_module::hruout_waterbal_ave' has no extracted documentation comment.
