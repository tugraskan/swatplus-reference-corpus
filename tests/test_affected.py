...
def test_declared_dependency_fanout_is_never_capped(tmp_path):
    store = FactStore(source_ref="t")
    store.add(sym("hub_module", hash="module-new"))
    pages = [page(tmp_path, "hub_module", "module-old")]
    for index in range(20):
        name = f"consumer_{index}"
        store.add(sym(name, hash="same", depends_on=["hub_module"]))
        pages.append(page(tmp_path, name, "same"))

    report = compute_status(store, pages, max_fanout=1)

    assert len(report.affected) == 20
    assert all(
        report.affected_by[f"consumer_{index}.md"]
        == ["hub_module (dependency)"]
        for index in range(20)
    )
