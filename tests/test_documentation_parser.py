...
def test_projection_preserves_comparison_parser_diagnostics(tmp_path):
    ...
    # A file whose signed operand was parenthesised for the parser is a second
    # kind of degradation-from-pristine-source, and travels with the fallbacks.
    assert store.normalized_files == {"signs.f90": [12, 40]}
    assert rich.index.metadata[FPARSER_DIAGNOSTICS_METADATA_KEY] == {
        "fallback_files": ["broken.f90"],
        "normalized_files": {"signs.f90": [12, 40]},
        "parse_errors": {"broken.f90": "FortranSyntaxError: bad expression"},
        "fparser_version": fparser_version(),
    }
    snapshot = tmp_path / "rich.json"
    rich.save(snapshot)
    reprojected = project_documentation_facts(RichStore.load(snapshot), FIXTURES, "source-sha")
    assert reprojected.parse_errors == store.parse_errors
    assert reprojected.fallback_files == store.fallback_files
    assert reprojected.normalized_files == store.normalized_files
